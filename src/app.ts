...
function updateDirectoryFilterToggleState() {
  ...
  if (chevron) chevron.style.transform = directoryFiltersOpen ? 'rotate(180deg)' : 'rotate(0deg)';
}
...
function renderDirectoryFilterControls(profiles = getDirectoryProfiles()) {
  DIRECTORY_FILTER_GROUPS.forEach(group => {
    ...
    activeDirectoryFilters[group.key] = activeDirectoryFilters[group.key].filter(selected =>
      options.some(option => normalizeDirectoryFilterValue(option) === normalizeDirectoryFilterValue(selected))
    );
...
function profileMatchesReference(profile = {}, reference = {}) {
  return !!(
    (reference?.id && String(profile?.id) === String(reference.id)) ||
...
function getPendingCesChangeRequestForProfile(profile = {}) {
  return getSecurityLogEntries().find(entry =>
    ...
    profileMatchesReference(entry?.profileRef || {}, profile)
  ) || null;
}
...
function isCESNumberAvailable(value, currentProfile = null, pendingRequestIdToIgnore = '') {
  ...
  if (takenByProfile) return false;
  const takenByPendingRequest = getSecurityLogEntries().some(entry => {
    if (entry?.type !== 'ces_change_request' || entry?.status !== 'pending') return false;
    if (pendingRequestIdToIgnore && String(entry.id) === String(pendingRequestIdToIgnore)) return false;
...
function setView(name, btn) {
  ...
  if (name === 'charter') { setTimeout(() => { renderCharterFlowActions(); renderCharterCodesAccordion(); }, 50); }
...
function setPortfolioUploadingState(isUploading) {
  ...
  if (portfolioUploadCount < 0) portfolioUploadCount = 0;
  updateSubmitBtn();
}
...
async function deleteUploadedPortfolioAsset(item) {
  if (!isStoredPortfolioItem(item)) return;
...
async function handlePortfolioFileSelection(index, event) {
  ...
  if (!file || !Array.isArray(formState.portfolioItems) || !formState.portfolioItems[index]) return;
...
function renderAgreementPortalPhaseOptions(selected = '') {
  return AGREEMENT_PORTAL_PHASES.map(phase => (
    `<option value="${escapeHtml(phase)}"${selected === phase ? ' selected' : ''}>${escapeHtml(phase)}</option>`
  )).join('');
}
...
function updateSubmitBtn() {
  ...
  const cesOk = cesVal.length === 9 && !quadRepeat && (
    (!currentUser && isCESNumberAvailable(cesVal)) ||
    (currentUser && (
      cesVal === String(currentUser.cesNumber || '').trim() ||
...
function submitCESProfile() {
  ...
  if (existingProfile) {
    if ((document.getElementById('f_name')?.value || '').trim() !== String(existingProfile?.name || '').trim()) changes.push('name');
    if ((document.getElementById('f_title')?.value || '').trim() !== String(existingProfile?.title || '').trim()) changes.push('title');
    if ((document.getElementById('f_location')?.value || '').trim() !== String(existingProfile?.location || '').trim()) changes.push('location');
    if (normalizeAstrologyPlacement(document.getElementById('f_sun_placement')?.value || '') !== normalizeAstrologyPlacement(existingProfile?.sunPlacement || existingProfile?.sun || '')) changes.push('sun placement');
    if (normalizeAstrologyPlacement(document.getElementById('f_moon_placement')?.value || '') !== normalizeAstrologyPlacement(existingProfile?.moonPlacement || existingProfile?.moon || '')) changes.push('moon placement');
    if ((document.getElementById('f_heartlight')?.value || '').trim() !== String(existingProfile?.heartlight || '').trim()) changes.push('heartlight');
    if ((document.getElementById('f_timeline')?.value || '').trim() !== String(existingProfile?.timeline || '').trim()) changes.push('timeline');
    if ((document.getElementById('f_consent')?.value || '').trim() !== String(existingProfile?.consent || '').trim()) changes.push('consent');
    if ((document.getElementById('f_portfolio')?.value || '').trim() !== String(existingProfile?.portfolioLink || existingProfile?.portfolio || '').trim()) changes.push('portfolio link');
    if (JSON.stringify(portfolioItems) !== JSON.stringify(normalizePortfolioItems(existingProfile?.portfolioItems || existingProfile?.mediaPortfolio || []))) changes.push('portfolio media');
    if (JSON.stringify(contactMethods) !== JSON.stringify(normalizeContactMethods(existingProfile?.contactMethods, existingProfile?.contactMethod || existingProfile?.contact || ''))) changes.push('contact paths');
    if (JSON.stringify(contactVisibility) !== JSON.stringify(normalizeContactVisibility(existingProfile?.contactVisibility, existingProfile?.contactMethods || {}, !!existingProfile?.publicContactVisibility))) changes.push('public contact visibility');
    if (JSON.stringify(rays) !== JSON.stringify(asArray(existingProfile?.rays))) changes.push('ray resonance');
    if (JSON.stringify([...formState.selectedOfferings]) !== JSON.stringify(asArray(existingProfile?.offerings))) changes.push('offerings');
    if (JSON.stringify([...formState.selectedExchanges]) !== JSON.stringify(asArray(existingProfile?.exchanges && existingProfile?.exchanges.length ? existingProfile?.exchanges : existingProfile?.exchangePathways))) changes.push('exchange pathways');
    if (JSON.stringify(seasons) !== JSON.stringify({ Winter: false, Spring: false, Summer: false, Fall: false, ...(existingProfile?.seasons || {}) })) changes.push('seasonal capacity');
    if (JSON.stringify([...formState.selectedNumerology]) !== JSON.stringify(asArray(existingProfile?.numerology))) changes.push('numerology');
    if (JSON.stringify([...formState.selectedAccessibility]) !== JSON.stringify(asArray(existingProfile?.accessibility && existingProfile?.accessibility.length ? existingProfile?.accessibility : existingProfile?.accessibilityOfferings))) changes.push('accessibility');
    if (normalizeWishAvailability(formState.wishAvailability || existingProfile?.wishAvailability || existingProfile?.directoryWishStatus) !== normalizeWishAvailability(existingProfile?.wishAvailability || existingProfile?.directoryWishStatus)) changes.push('wish status');
    if (requestedCesChange) changes.push('C.E.S. number request');
  }
  const profile = {
    ...(existingProfile || {}),
...
async function removePortfolioMediaFromProfile(id, mediaIndex) {
  ...
  if (!collectionKey || !collectionList || profileIndex < 0) return;
  ...
  if (!confirm(`Remove ${targetItem.type === 'video' ? 'this video' : 'this photo'} from ${profile.name}'s portfolio?`)) return;
...
function approveCesChangeRequest(id) {
  ...
  if (request?.type !== 'ces_change_request' || request?.status !== 'pending') return;
...
function generateCESNumberValue() {
  ...
  getSecurityLogEntries()
    ...
    .forEach(entry => {
      ...
      if (requested) used.add(requested);
    });
