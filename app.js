const AGREEMENT_STORAGE_KEY = 'agreements';
const STORAGE_PREFIX = 'ai_';
const STORAGE_QUEUE_KEYS = new Set(['pending', 'approved', 'returned']);
...
function isQueueStorageKey(key) {
    return STORAGE_QUEUE_KEYS.has(key);
}
...
function stripDemoProfiles(list) {
...
function hasMeaningfulStorageValue(value) {
...
function shouldSeedLocalDemoData() {
...
function getStorage(key, fallback) {
    ...
    return value === null || value === undefined ? fallback : cloneStorageValue(value);
}
function persistStorageToRemote(key, value) {
...
async function fetchRemoteStorageValue(key) {
...
function getDemoPendingProfiles() {
    return [
...
function getCurrentUser() {
    try {
        ...
        if (!raw)
            return null;
        return normalizeCreatorRecord(JSON.parse(raw));
    }
    catch (_error) {
        return null;
    }
...
function initializeProfileStorageDefaults() {
    const authorized = getStorage('authorizedCES', []);
    if (authorized.length === 0) {
        setStorage('authorizedCES', [createDefaultStewardEntry()]);
        return;
    }
    let changed = false;
...
function syncCurrentUserWithStoredProfile() {
    const currentUser = getCurrentUser();
    if (!currentUser)
        return;
    const allProfiles = ['pending', 'approved', 'returned'].flatMap(key => getStorage(key, []));
    const matchedProfile = allProfiles.find(profile => profile.id === currentUser.id ||
        (currentUser.cesNumber && profile.cesNumber === currentUser.cesNumber));
    if (matchedProfile)
        setCurrentUser(matchedProfile);
}
function rerenderStorageBoundViews() {
    if (typeof renderProfileCorner === 'function')
        renderProfileCorner();
    ...
    if (typeof renderPendingList === 'function')
        renderPendingList();
    if (typeof renderApprovedList === 'function')
        renderApprovedList();
    if (typeof renderReturnedList === 'function')
        renderReturnedList();
    if (typeof renderAuthorizedList === 'function')
        renderAuthorizedList();
    if (typeof renderSecurityLog === 'function')
        renderSecurityLog();
}
async function hydrateStorageFromRemote() {
    if (storageHydrationPromise)
        return storageHydrationPromise;
    storageHydrationPromise = (async () => {
        const successfulReads = [];
        await Promise.allSettled(REMOTE_STORAGE_KEYS.map(async (key) => {
        ...
        successfulReads.forEach(({ key, value }) => {
            ...
            if (hasMeaningfulStorageValue(localValue)) {
                const normalizedLocal = writeLocalStorageValue(key, (localValue ?? null));
...
function normalizeAgreementRoles(value) {
    const roles = Array.isArray(value) ? value : [];
    const normalized = roles
        .map(role => ({
        label: String(role?.label || '').trim(),
        beings: String(role?.beings || '').trim(),
    }))
...
function normalizeAgreementRecord(record = {}) {
    ...
    return {
        ...
        sharedContactMethods: normalizeContactMethods(record.sharedContactMethods),
...
function getAllAgreements() {
    return getStorage(AGREEMENT_STORAGE_KEY, []).map(entry => normalizeAgreementRecord(entry));
}
function getAgreementById(id) {
    return getAllAgreements().find(entry => entry.id === id) || null;
}
function agreementMatchesProfile(agreement, profile = {}) {
    const match = getProfileConnectionMatch(profile);
    return ((match.id && (String(agreement.wishingProfileId || '') === match.id || String(agreement.coCreatorProfileId || '') === match.id)) ||
        (match.cesNumber && (String(agreement.wishingCES || '') === match.cesNumber || String(agreement.coCreatorCES || '') === match.cesNumber)));
}
function getProfileAgreements(profile = {}) {
    return getAllAgreements()
        .filter(entry => agreementMatchesProfile(entry, profile))
        .sort((a, b) => String(b.updatedAt || '').localeCompare(String(a.updatedAt || '')));
}
function getAgreementCounterparty(agreement, profile = {}) {
    const match = getProfileConnectionMatch(profile);
    const isWishingBeing = (match.id && String(agreement.wishingProfileId || '') === match.id) ||
...
function getAgreementStatusMarkup(agreement) {
...
function normalizeSeasons(profile = {}) {
    const base = { Winter: false, Spring: false, Summer: false, Fall: false };
    if (profile.seasons && typeof profile.seasons === 'object') {
        return { ...base, ...profile.seasons };
    }
...
function normalizeWishAvailability(value) {
    return value === 'full' ? 'full' : 'accepting';
}
function getWishAvailabilityMeta(value) {
...
function renderWishAvailabilityBadge(value) {
...
function normalizeCreatorRecord(profile = {}) {
    const rayKey = String(profile.ray || String(profile.primaryRay || '').replace(/\s+Ray$/, '') || '');
    const rays = Array.isArray(profile.rays) && profile.rays.length ? profile.rays : (rayKey ? [rayKey] : []);
    ...
    const contactMethods = normalizeContactMethods(profile.contactMethods, legacyContactText);
    const contactVisibility = normalizeContactVisibility(profile.contactVisibility, contactMethods, !!(profile.publicContactVisibility || profile.contactVisibility === 'public'));
    const publicContactVisibility = getPublicContactMethodItems(contactMethods, contactVisibility).length > 0;
    const contactMethodItems = getContactMethodItems(contactMethods);
    const stewardStanding = ['suspended', 'banned'].includes(String(profile.stewardStanding || '').trim())
        ? String(profile.stewardStanding || '').trim()
        : 'active';
    return {
        ...profile,
        id: profile.id,
        ...
        offerings: asArray(profile.offerings),
        exchanges: asArray(profile.exchanges && profile.exchanges.length ? profile.exchanges : profile.exchangePathways),
        seasons,
        timeline: String(profile.timeline || profile.coCreationTimeline || ''),
        numerology: asArray(profile.numerology),
        accessibility: asArray(profile.accessibility && profile.accessibility.length
        ...
        stewardAlerts: Array.isArray(profile.stewardAlerts)
            ? profile.stewardAlerts.map(alert => ({
                id: String(alert?.id || ''),
                kind: String(alert?.kind || '').trim() === 'audit_notice' ? 'audit_notice' : 'media_removed',
...
function getStandingFromAuditOutcome(outcome = '') {
    ...
    if (normalized === 'long-term suspension')
        return 'suspended';
    if (normalized === 'banning')
        return 'banned';
    return 'active';
}
function getStandingLabel(standing = '') {
    if (standing === 'suspended')
        return 'Long-term suspension';
    if (standing === 'banned')
        return 'Banned from the Exchange';
    return 'Active';
}
function createAuditNoticeAlert({ title = '', message = '', createdBy = 'Steward', auditId = '', auditStatus = 'opened', auditOutcome = '', auditStanding = 'active', } = {}) {
    return {
...
function guessLegacyContactMethods(legacyValue) {
    ...
    if (!text)
        return methods;
    if (/instagram\.com|^@[\w.]+$/i.test(text))
        methods.instagram = text;
    else if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(text))
        methods.email = text;
    else if (/discord/i.test(text) || /#[0-9]{4}$/.test(text))
        methods.discord = text.replace(/^discord[:\s-]*/i, '').trim();
    else if (/signal/i.test(text))
        methods.signal = text.replace(/^signal[:\s-]*/i, '').trim();
    else if (/whatsapp/i.test(text))
        methods.whatsapp = text.replace(/^whatsapp[:\s-]*/i, '').trim();
    else if (/[0-9]{7,}/.test(text))
        methods.phone = text;
    return methods;
}
...
function sanitizePhoneNumber(value, preservePlus = false) {
    ...
    if (!digits)
        return '';
    if (preservePlus && raw.startsWith('+'))
        return `+${digits}`;
    return digits;
}
function getContactMethodItems(contactMethods = {}) {
    return CONTACT_FIELD_CONFIG.map(field => {
        ...
        if (field.key === 'phone') {
            ...
            const actions = [];
            if (tel)
                actions.push({ label: 'Call', href: `tel:${tel}` });
            if (tel)
                actions.push({ label: 'Message', href: `sms:${tel}` });
...
function renderContactPathChips(contactMethods = {}) {
    const items = getContactMethodItems(contactMethods);
    if (!items.length)
        return '<span class="preview-chip" style="opacity:0.4;font-style:italic">No public contact channels selected yet</span>';
...
function setView(name, btn) {
    ...
    if (jc)
        jc.style.display = name === 'directory' ? 'block' : 'none';
    if (name === 'submit') {
        setTimeout(initCESForm, 50);
    }
    if (name === 'wish') {
        setTimeout(() => { resetWishCastSection(); buildWishForm(); renderWishField(); setTimeout(focusWishFieldSection, 40); }, 50);
    }
    if (name !== 'wish') {
        resetWishCastSection();
    }
    if (name === 'wishes') {
...
function closeAgreementOutside(e) {
    if (e.target === document.getElementById('agreementOverlay'))
        closeAgreement();
}
...
function syncAgreementToConnections(agreement, currentUser) {
    if (!currentUser || !agreement || !['directory', 'wish'].includes(agreement.sourceType))
        return;
    ...
    if (!otherProfileId && !otherCES && !otherName)
        return;
    ...
    if (matchIndex >= 0)
        existingConnections[matchIndex] = nextEntry;
    else
        existingConnections.push(nextEntry);
...
function getVisibleContactMethods(contactMethods = {}, contactVisibility = {}) {
    ...
    CONTACT_FIELD_KEYS.forEach(key => {
        if (normalizedVisibility[key])
            visible[key] = String(contactMethods[key] || '').trim();
    });
    return visible;
}
...
function buildEmojiPicker() {
    ...
    if (input && input.value !== formState.emoji)
        input.value = formState.emoji;
...
function toggleRay(key) {
    ...
    if (idx > -1) {
        ...
        if (formState.selectedRays.length >= 3)
            return;
...
function buildChipGrid(containerId, presets, stateKey) {
    ...
    g.innerHTML = options.map(p => `<div class="chip-opt ${selected.includes(p) ? 'selected' : ''}" onclick="toggleChip('${stateKey}','${p.replace(/'/g, "\\'")}',this)">${p}</div>`).join('');
}
...
function addCustomChip(type) {
    ...
    if (!formState[mapCustom[type]])
        formState[mapCustom[type]] = [];
    ...
    if (g) {
        ...
        g.innerHTML = allOpts.map(p => `<div class="chip-opt ${sel.includes(p) ? 'selected' : ''}" onclick="toggleChip('${mapState[type]}','${p.replace(/'/g, "\\'")}',this)">${p}</div>`).join('');
    }
    updatePreview();
}
...
function updatePortfolioItem(index, key, value) {
    if (!Array.isArray(formState.portfolioItems) || !formState.portfolioItems[index])
        return;
...
function buildExchangeToggles() {
    ...
    if (!g)
        return;
    g.innerHTML = EXCHANGE_PRESETS.map(e => `<div class="exchange-toggle ${formState.selectedExchanges.includes(e) ? 'selected' : ''}" onclick="toggleExchange('${e}',this)">${e}</div>`).join('');
}
function toggleExchange(val, el) {
    const idx = formState.selectedExchanges.indexOf(val);
    if (idx > -1) {
        formState.selectedExchanges.splice(idx, 1);
        el.classList.remove('selected');
    }
    else {
        formState.selectedExchanges.push(val);
        el.classList.add('selected');
    }
    updatePreview();
}
...
function buildSeasonToggles() {
    ...
    if (!g)
        return;
    g.innerHTML = Object.keys(formState.selectedSeasons).map(s => `<div class="season-toggle ${formState.selectedSeasons[s] ? 'open' : ''}" style="--s-color:${SEASON_COLORS[s]}" onclick="toggleSeason('${s}',this)">
...
function toggleOath() {
    ...
    if (box)
        box.classList.toggle('checked', formState.oathSigned);
    updateSubmitBtn();
}
function toggleOath2() {
    ...
    if (box)
        box.classList.toggle('checked', formState.oath2Signed);
    updateSubmitBtn();
}
...
function updatePreview() {
    ...
    if (bar)
        bar.style.background = gradBar;
    if (glow)
        glow.style.background = gradGlow;
    if (card) {
    ...
    if (!activePhoto) {
        ...
        const fsize = '1.8rem';
        if (phEl) {
            phEl.textContent = display;
            phEl.style.fontSize = fsize;
        }
        if (pvPh) {
            pvPh.textContent = display;
            pvPh.style.fontSize = '1.3rem';
        }
    }
    updateSubmitBtn();
    const pTitle = document.getElementById('previewTitle');
    if (pTitle)
        pTitle.textContent = title || 'Creator Title';
    ...
    if (pSeas) {
        pSeas.innerHTML = Object.entries(formState.selectedSeasons).map(([s, open]) => `<div class="preview-season-cell ${open ? 'open' : 'closed'}" style="${open ? `border-color:${c1}40;background:${c1}08` : ''}">
    ...
    if (pEx) {
        if (formState.selectedExchanges.length === 0) {
            ...
            pEx.innerHTML = formState.selectedExchanges.map(e => `<span class="preview-exchange-chip" style="border-color:${c1}60;color:${c1}">${e}</span>`).join('');
        }
    }
...
async function submitCES() {
    if (!formState.oathSigned || !formState.oath2Signed)
        return;
    const name = document.getElementById('f_name')?.value?.trim();
    if (!name)
        return;
...
function summarizeWish(text, max = 170) {
    ...
    if (!clean)
        return 'This wish is still taking form in the field.';
...
function renderWishRoleList(entry) {
    ...
    return entry.roles.map(role => {
        ...
        const metaParts = [];
        if (namedBeings.length)
            metaParts.push(`${namedBeings.length} being${namedBeings.length === 1 ? '' : 's'} already woven`);
        if (openInvites)
            metaParts.push(`${openInvites} open invitation${openInvites === 1 ? '' : 's'}`);
        if (!metaParts.length)
            metaParts.push('Open to resonance');
        const chips = [
...
function renderWishField() {
    ...
    if (rolesEl)
        rolesEl.textContent = String(totalRoles);
    if (multiEl)
        multiEl.textContent = String(multiBeingRoles);
    if (resultsCopy) {
...
function renderMyWishes() {
    ...
    if (!content || !awaitingList || !grantedList)
        return;
    const user = getCurrentUser();
    if (!user) {
        if (totalEl)
    ...
    const granted = wishes.filter(entry => entry.status === 'granted');
    if (totalEl)
        totalEl.textContent = String(wishes.length);
    if (awaitingEl)
        awaitingEl.textContent = String(awaiting.length);
    if (grantedEl)
        grantedEl.textContent = String(granted.length);
    if (!wishes.length) {
...
function buildWishForm() {
    ...
    if (wtg)
        wtg.innerHTML = WISH_TYPES.map(t => `<div class="wish-type-chip ${wishState.selectedWishTypes.includes(t) ? 'selected' : ''}" onclick="toggleWishType('${t}',this)">${t}</div>`).join('');
    // Heart Themes
    const htg = document.getElementById('heartThemeGrid');
    if (htg)
        htg.innerHTML = HEART_THEMES.map(h => `<div class="heart-theme-btn ${wishState.selectedHeartTheme === h.l ? 'selected' : ''}" onclick="selectHeartTheme('${h.l}',this)">
    ...
    if (fmtg)
        fmtg.innerHTML = WISH_FORMATS.map(f => `<div class="format-btn ${wishState.selectedFormats.includes(f) ? 'selected' : ''}" onclick="toggleWishFormat('${f}',this)">${f}</div>`).join('');
    // Exchange
    const exg = document.getElementById('wishExchangeGrid');
    if (exg)
        exg.innerHTML = EXCHANGE_PRESETS.map(e => `<div class="exchange-toggle ${wishState.selectedExchanges.includes(e) ? 'selected' : ''}" onclick="toggleWishExchange('${e}',this)">${e}</div>`).join('');
    // Ray mini-pills
    const rg = document.getElementById('wishRayGrid');
    if (rg)
        rg.innerHTML = ALL_RAYS.map(r => `<div style="display:inline-flex;align-items:center;gap:0.3rem;padding:0.25rem 0.65rem;border-radius:2rem;border:1.5px solid ${wishState.selectedRay === r.key ? r.color : 'rgba(255,255,255,0.1)'};background:${wishState.selectedRay === r.key ? r.color + '18' : 'rgba(255,255,255,0.02)'};cursor:pointer;font-size:0.68rem;color:${wishState.selectedRay === r.key ? r.color : 'rgba(240,232,213,0.55)'};transition:all 0.2s" onclick="selectWishRay('${r.key}')">
...
function renderWishProfileStatus() {
    ...
    if (nameInput)
        nameInput.value = user ? [user.name, user.pronouns].filter(Boolean).join(' · ') : '';
    if (locationInput)
...
function toggleWishType(val, el) {
    ...
    if (idx > -1) {
        wishState.selectedWishTypes.splice(idx, 1);
        el.classList.remove('selected');
    }
    else {
        wishState.selectedWishTypes.push(val);
        el.classList.add('selected');
    }
    updateWishPreview();
}
function selectHeartTheme(val, el) {
    ...
    if (wishState.selectedHeartTheme)
        el.classList.add('selected');
    updateWishPreview();
}
function toggleWishFormat(val, el) {
    ...
    if (idx > -1) {
        wishState.selectedFormats.splice(idx, 1);
        el.classList.remove('selected');
    }
    else {
        wishState.selectedFormats.push(val);
        el.classList.add('selected');
    }
    updateWishPreview();
}
function toggleWishExchange(val, el) {
    ...
    if (idx > -1) {
        wishState.selectedExchanges.splice(idx, 1);
        el.classList.remove('selected');
    }
    else {
        wishState.selectedExchanges.push(val);
        el.classList.add('selected');
    }
    updateWishPreview();
}
...
function updateWishPreview() {
    ...
    if (pLetter)
        pLetter.textContent = letter ? '"' + letter + '"' : 'Your wish will appear here as you write…';
    const pPortal = document.getElementById('wPreviewPortal');
    if (pPortal)
        pPortal.textContent = portal || '—';
...
function submitWish() {
    ...
    if (!letter) {
        alert('✦ Please write your wish before sending it.');
        return;
    }
...
function closeCharterHexModalOutside(event) {
    if (event.target === document.getElementById('charterHexModalOverlay'))
        closeCharterHexModal();
}
...
// ══════════════════════════════════════════════════════
const _sp = ['aGVhcnRs', 'aWdodA=='];
const STEWARD_PASS = atob(_sp.join(''));
...
function syncOathBoxes() {
    ...
    if (oath1)
        oath1.classList.toggle('checked', !!formState.oathSigned);
    if (oath2)
        oath2.classList.toggle('checked', !!formState.oath2Signed);
}
function syncPhotoPreview(photoUrl) {
    ...
    if (placeholder) {
        ...
        if (!photoUrl)
            placeholder.textContent = normalizeAvatarMark(formState.emoji || '') || '✦';
    }
    if (nameEl)
        nameEl.textContent = photoUrl ? '✓ Current signature photo' : '';
}
...
function submitCESProfile() {
    ...
    for (const [key, list] of Object.entries(collections)) {
        const found = list.find(profile => (currentUser && profile.id === currentUser.id) ||
    ...
    Object.keys(collections).forEach(key => {
        collections[key] = collections[key].filter(entry => entry.id !== profile.id && (!profile.cesNumber || entry.cesNumber !== profile.cesNumber));
    });
...
function renderPendingList() {
    ...
    const pending = getStorage('pending', []);
    if (!pending.length) {
        list.innerHTML = '<div class="admin-empty">✦ No submissions awaiting review. The queue is clear and the field is held. ✦</div>';
        return;
    }
    list.innerHTML = pending.map((p, i) => adminCardHTML(p, `<input class="admin-return-note" type="text" id="note_${p.id}" placeholder="Steward note for return (optional)…">
     <button class="admin-btn-accept" onclick="acceptProfile('${p.id}')">✓ Accept</button>
     <button class="admin-btn-return" onclick="returnProfile('${p.id}')">↩ Return</button>`)).join('');
}
function renderApprovedList() {
    ...
    const approved = getStorage('approved', []);
    if (!approved.length) {
        list.innerHTML = '<div class="admin-empty">✦ The Directory awaits its first Co-Creators. ✦</div>';
        return;
    }
    list.innerHTML = approved.map(p => adminCardHTML(p, `<button class="admin-btn-return" onclick="removeFromDirectory('${p.id}')">Remove from Directory</button>`)).join('');
}
function renderReturnedList() {
    ...
    const returned = getStorage('returned', []);
    if (!returned.length) {
        list.innerHTML = '<div class="admin-empty">✦ No returned submissions. ✦</div>';
        return;
    }
    list.innerHTML = returned.map(p => adminCardHTML(p, `<button class="admin-btn-accept" onclick="reAcceptProfile('${p.id}')">Accept Now</button>`)).join('');
}
...
function cesDecrypt(encrypted) {
    ...
    if (!encrypted || !encrypted.startsWith('-'))
        return '';
...
function validateCESCode() {
    ...
    if (welcome && steward)
        welcome.textContent = 'Welcome, ' + steward.name;
...
function attemptPrimaryOverride() {
    ...
    if (val === STEWARD_PASS) {
        ...
        input.value = '';
        if (err) {
            err.textContent = 'Passphrase not recognized.';
            err.style.display = 'block';
        }
    }
}
...
function openCesAuditFromReport(reportId) {
    ...
    if (report?.type !== 'audit_report' || report?.status !== 'pending')
        return;
...
function closeCesAudit(auditId) {
    ...
    if (audit?.type !== 'ces_audit' || audit?.status !== 'opened')
        return;
    const now = new Date().toISOString();
...
function dismissAuditReport(reportId) {
    ...
    if (report?.type !== 'audit_report' || report?.status !== 'pending')
        return;
    if (!confirm('Dismiss this Report Audit without opening a C.E.S. Audit?'))
        return;
    log[reportIndex] = {
        ...report,
        status: 'dismissed',
...
function saveEditPassphrase(i) {
    ...
    if (!f1.value.trim()) {
        showMsg('Please enter a new passphrase.', '#e8a0a0');
        return;
    }
    if (f1.value !== f2.value) {
        showMsg('Passphrases do not match.', '#e8a0a0');
        return;
    }
    if (f1.value.trim().length < 4) {
        showMsg('Passphrase must be at least 4 characters.', '#e8a0a0');
        return;
    }
    ...
    const authorized = getStorage('authorizedCES', []);
    if (!authorized[i]) {
        showMsg('Steward not found.', '#e8a0a0');
        return;
    }
...
function addAuthorizedCES() {
    ...
    const authorized = getStorage('authorizedCES', []);
    if (authorized.some(e => e.cesPlain === cesVal)) {
        alert('This C.E.S. code is already registered.');
        return;
    }
...
function revokeCES(idx) {
    ...
    if (!entry)
        return;
    if (!confirm('Revoke Steward access for ' + entry.name + '? They will no longer be able to enter the Steward Field.'))
        return;
    // Log the revocation
    const log = getStorage('securityLog', []);
    log.unshift({
...
function generateCESNumberValue() {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
        ...
        while (value.length < 9) {
            ...
            if (recent.length === 3 && recent.split('').every(digit => digit === nextDigit))
                continue;
            value += nextDigit;
        }
        if (!used.has(value))
